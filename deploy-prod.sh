...
if ! kubectl get deployment apigateway-deployment &> /dev/null; then
  echo "Create new kubernetes deployment resources..."
  kubectl create -f ./deployment-prod.yml
else
  echo "Apply new docker image "
  kubectl set image deployments/apigateway-deployment apigateway-service=$ECR_REGISTRY/$AWS_ECR_REPOSITORY:$IMAGE_TAG --record
fi
...
if ! kubectl get deployment apigateway-deployment &> /dev/null; then
  ...
  if ! kubectl rollout status deployment/apigateway-deployment --watch=false | grep -q "successfully rolled out"; then
    echo "Rolling update has failed. Initiating rollback..."

    kubectl rollout undo deployment/apigateway-deployment
    kubectl rollout status deployment/apigateway-deployment --watch=true
    ...
    if ! kubectl get service apigateway-service &> /dev/null; then
